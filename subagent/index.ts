...
async function runSingleAgent(
    ...
    const exitCode = await new Promise<number>((resolve) => {
        ...
        const failForMalformedStdout = (line: string, error: unknown) => {
            const parseError = error instanceof Error ? error.message : String(error);
            const preview = line.length > 500 ? `${line.slice(0, 500)}...` : line;
            protocolError = `Malformed subagent stdout JSON: ${parseError}\nUnexpected stdout: ${preview}`;
            currentResult.errorMessage = protocolError;
            currentResult.stderr += `${protocolError}\n`;
        ...
        const processLine = (line: string) => {
            ...
            try {
                ...
                failForMalformedStdout(line, error);
                return;
            }
        ...
        proc.on("close", (code) => {
            ...
            resolve(protocolError ? 1 : (code ?? 0));
        });
...
export default function (pi: ExtensionAPI) {
    registerTool(pi, {
        ...
        description: [
            ...
            'Includes packaged default agents, plus user agents from ~/.pi/agent/agents by default.',
            'To enable project-local agents in .pi/agents, set agentScope: "both" (or "project").',
        ].join(" "),
        promptSnippet: "Delegate work to specialized subagents with isolated context",
        promptGuidelines: [
            "Use this tool when the task benefits from isolated scout/planner/worker/reviewer passes or parallel delegation.",
            "Use chain mode when later steps should consume earlier output via {previous}; use parallel mode only for independent tasks.",
            "Keep delegated tasks specific and self-contained, and prefer the current cwd unless a different step cwd is required.",
        ],
        parameters: SubagentParams,

        async execute(_toolCallId, params: Static<typeof SubagentParams>, signal, onUpdate, ctx) {
