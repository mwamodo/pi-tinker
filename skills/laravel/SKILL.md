...
### 1. Database Performance → [reference guide](references/db-performance.md)
...
### 2. Advanced Query Patterns → [reference guide](references/advanced-queries.md)
...
### 3. Security → [reference guide](references/security.md)
...
### 4. Caching → [reference guide](references/caching.md)
...
### 5. Eloquent Patterns → [reference guide](references/eloquent.md)
...
- Never hardcode table names except in migrations — use `(new Model)->getTable()` or Eloquent queries

### 6. Validation & Forms → [reference guide](references/validation.md)

- Form Request classes, not inline validation
- Array notation `['required', 'email']` for new code; follow existing convention for consistency
- `$request->validated()` only — never `$request->all()`
- `Rule::when()` for conditional validation
- `after()` instead of `withValidator()`

### 7. Configuration → [reference guide](references/config.md)

- `env()` only inside config files
- `App::environment()` or `app()->isProduction()`
- Config, lang files, and constants over hardcoded text

### 8. Testing Patterns → [reference guide](references/testing.md)
...
### 9. Queue & Job Patterns → [reference guide](references/queue-jobs.md)
...
### 10. Routing & Controllers → [reference guide](references/routing.md)
...
### 11. HTTP Client → [reference guide](references/http-client.md)
...
### 12. Events → [reference guide](references/events-notifications.md)
...
### 13. Notifications → [reference guide](references/events-notifications.md)
...
### 14. Mail → [reference guide](references/mail.md)

- Queue mailables with `ShouldQueue`
- `assertQueued()` not `assertSent()` for queued mailables
- Markdown mailables for transactional emails

### 15. Error Handling → [reference guide](references/error-handling.md)
...
### 16. Task Scheduling → [reference guide](references/scheduling.md)
...
### 17. Architecture → [reference guide](references/architecture.md)
...
### 18. Migrations → [reference guide](references/migrations.md)
...
### 19. Collections → [reference guide](references/collections.md)
...
### 20. Blade & Views → [reference guide](references/blade-views.md)
...
### 21. Conventions & Style → [reference guide](references/style.md)
...
Always use a `scout` sub-agent to read rule files and explore this skill's content.

1. Identify the file type and select relevant sections (e.g., migration → §18, controller → §1, §3, §5, §6, §10)
